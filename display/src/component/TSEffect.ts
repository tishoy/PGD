...
module TSEffect {
	...
	export function blinkEffect(obj, interval: number = 1000): void {
		new BlinkEffect(obj, interval);
	}

	export function breathEffect(obj, interval: number = 1000): BreathEffect {
		return new BreathEffect(obj, interval);
	}

	export function splash(): egret.Tween {
		return
	}

	export class CooldownEffect extends egret.Sprite {
		...
		public draw(e: Event): void {
			this.clear();
			var timePast: number = egret.getTimer() - this.startTime - this.delay;
